...
class PolicyIteration:
    def __init__(self, env, theta, gamma):
        ...
        self.pi = [[1 / 4 for _ in range(4)] for _ in range(self.state_num)]
    ...
    def policy_evaluation(self):
        while True:
            ...
            for s in range(self.state_num):
                v = 0
                for a, prob in enumerate(self.pi[s]):
                    for p, s_n, r, d in self.env.P[s][a]:
                        v += prob * p * (r + self.gamma * self.v[s_n] * (1 - d))
                new_v[s] = v
    ...
    def policy_improvement(self):
        ...
        for s in range(self.state_num):
            ...
            for a in range(4):
                ...
                q_list.append(q)
            max_q = max(q_list)
            count = q_list.count(max_q)
            self.pi[s] = [1 / count if q == max_q else 0 for q in q_list]
            if np.argmax(self.pi[s]) != old_a:
                stable = False
        return stable
